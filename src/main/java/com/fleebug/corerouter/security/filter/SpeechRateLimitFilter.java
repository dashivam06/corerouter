...
public class SpeechRateLimitFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (HttpMethod.POST.matches(method) && path.equals(ApiPaths.SPEECH_TO_TEXT_JOBS)) {
            String token = extractToken(request);
            if (token != null) {
                String apiKeyHash = apiKeyService.hashKey(token);
                Bucket bucket = redisBucketService.resolveSpeechApiKeyBucket(apiKeyHash);
                ...
                if (!probe.isConsumed()) {
                    ...
                    telemetryClient.trackTrace("Speech-to-text create rate limit exceeded", SeverityLevel.Information, properties);

                    writeTooManyRequests(response, request,
                            "Speech-to-text creation rate limit exceeded (5 requests per minute). Retry after " + retryAfter + " seconds.");
                    return;
                }
            }
        }
